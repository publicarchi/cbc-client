<script>
	...
	import { DirectionLoopLeft } from 'carbon-icons-svelte'
		...
		>
			{#if checked.length > 0}
				{#each checked as tag}
					<ListBoxSelection
						...
						on:clear
						on:clear={(e) => {
							let tag_i = sortedItems.findIndex((item) => item.id === tag.id)
							sortedItems[tag_i].checked = false
			...
			{#if filterable}
				<input
					...
					on:paste
					{disabled}
					placeholder={checked.length === 0 ? placeholder : ''}
					{id}
					{name}
				/>
				{#if invalid}
				...
				{#if checked.length > 0}
					<ListBoxSelection
						...
						{disabled}
						{open}
						on:clear={() => (sortedItems = sortedItems.map((_) => ({ ..._, checked: false })))}
					/>
				{/if}
				<ListBoxMenuIcon
